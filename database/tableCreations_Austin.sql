-- MySQL Script generated by MySQL Workbench
-- Mon Dec  4 16:00:57 2017
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';
SET FOREIGN_KEY_CHECKS = 0;
-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
-- -----------------------------------------------------
-- Schema adase1db
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema adase1db
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `adase1db` DEFAULT CHARACTER SET latin1 ;
USE `adase1db` ;

-- -----------------------------------------------------
-- Table `adase1db`.`Amenity`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Amenity` (
  `amenityid` INT(11) NOT NULL,
  `amenity` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`amenityid`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Applicant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Applicant` (
  `ssn` INT(11) NOT NULL,
  `l_name` VARCHAR(45) NULL DEFAULT NULL,
  `m_name` VARCHAR(45) NULL DEFAULT NULL,
  `creditScore` VARCHAR(45) NULL DEFAULT NULL,
  `address` VARCHAR(45) NULL DEFAULT NULL,
  `moveDate` VARCHAR(45) NULL DEFAULT NULL,
  `phoneNumber` VARCHAR(45) NULL DEFAULT NULL,
  `employer` VARCHAR(45) NULL DEFAULT NULL,
  `jobTitle` VARCHAR(45) NULL DEFAULT NULL,
  `salary` VARCHAR(45) NULL DEFAULT NULL,
  `f_name` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`ssn`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Tennant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Tennant` (
  `ssn` INT(11) NOT NULL,
  `f_name` VARCHAR(45) NULL DEFAULT NULL,
  `m_name` VARCHAR(45) NULL DEFAULT NULL,
  `l_name` VARCHAR(45) NULL DEFAULT NULL,
  `birthdate` DATETIME NULL DEFAULT NULL,
  `phoneNumber` VARCHAR(45) NULL DEFAULT NULL,
  `car` VARCHAR(45) NULL DEFAULT NULL,
  `workStatus` VARCHAR(45) NULL DEFAULT NULL,
  `rentersInsurance` VARCHAR(45) NULL DEFAULT NULL,
  `rent` DECIMAL(13,2) NULL DEFAULT NULL,
  `leaseEndDate` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`ssn`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Property`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Property` (
  `propertyID` INT(11) NOT NULL,
  `deckOrPatio` VARCHAR(45) NULL DEFAULT NULL,
  `washer` VARCHAR(45) NULL DEFAULT NULL,
  `dryer` VARCHAR(45) NULL DEFAULT NULL,
  `offStreetParking` VARCHAR(45) NULL DEFAULT NULL,
  `numberOfBaths` INT(11) NULL DEFAULT NULL,
  `buildingType` VARCHAR(45) NULL DEFAULT NULL,
  `phoneNumber` VARCHAR(45) NULL DEFAULT NULL,
  `basement` VARCHAR(45) NULL DEFAULT NULL,
  `parkingType` VARCHAR(45) NULL DEFAULT NULL,
  `numOfRooms` VARCHAR(45) NULL DEFAULT NULL,
  `zipCode` INT(11) NULL DEFAULT NULL,
  `streetAddress` VARCHAR(45) NULL DEFAULT NULL,
  `city` VARCHAR(45) NULL DEFAULT NULL,
  `state` VARCHAR(45) NULL DEFAULT NULL,
  `numOfTennants` INT(11) NULL DEFAULT NULL,
  `Tennant_ssn` INT(11) NOT NULL,
  PRIMARY KEY (`propertyID`, `Tennant_ssn`),
  INDEX `fk_Property_Tennant_idx` (`Tennant_ssn` ASC),
  CONSTRAINT `fk_Property_Tennant`
  FOREIGN KEY (`Tennant_ssn`)
  REFERENCES `adase1db`.`Tennant` (`ssn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Applicant_Applies_For_Property`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Applicant_Applies_For_Property` (
  `Applicant_ssn` INT(11) NOT NULL,
  `Property_propertyID` INT(11) NOT NULL,
  `applicationStatus` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`Applicant_ssn`, `Property_propertyID`),
  INDEX `fk_Applicant_has_Property_Applicant1_idx` (`Applicant_ssn` ASC),
  INDEX `fk_Applicant_has_Property_Property1_idx` (`Property_propertyID` ASC),
  CONSTRAINT `fk_Applicant_has_Property_Applicant1`
  FOREIGN KEY (`Applicant_ssn`)
  REFERENCES `adase1db`.`Applicant` (`ssn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Applicant_has_Property_Property1`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Dependent`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Dependent` (
  `depId` INT(11) NOT NULL,
  `f_name` VARCHAR(45) NULL DEFAULT NULL,
  `m_name` VARCHAR(45) NULL DEFAULT NULL,
  `l_name` VARCHAR(45) NULL DEFAULT NULL,
  `birthDate` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`depId`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Dependent_lives_with_Tennant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Dependent_lives_with_Tennant` (
  `Dependent_depId` INT(11) NULL DEFAULT NULL,
  `Tennant_ssn` INT(11) NOT NULL,
  PRIMARY KEY (`Tennant_ssn`),
  INDEX `fk_Dependent_has_Tennant_Tennant1_idx` (`Tennant_ssn` ASC),
  INDEX `fk_Dependent_has_Tennant_Dependent1_idx` (`Dependent_depId` ASC),
  CONSTRAINT `fk_Dependent_has_Tennant_Dependent1`
  FOREIGN KEY (`Dependent_depId`)
  REFERENCES `adase1db`.`Dependent` (`depId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Dependent_has_Tennant_Tennant1`
  FOREIGN KEY (`Tennant_ssn`)
  REFERENCES `adase1db`.`Tennant` (`ssn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Employee`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Employee` (
  `ssn` INT(11) NOT NULL,
  `f_name` VARCHAR(45) NULL DEFAULT NULL,
  `m_name` VARCHAR(45) NULL DEFAULT NULL,
  `l_name` VARCHAR(45) NULL DEFAULT NULL,
  `pay` DECIMAL(13,2) NULL DEFAULT NULL,
  `phoneNumber` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`ssn`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Employee_Works_At_Property`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Employee_Works_At_Property` (
  `Property_propertyID` INT(11) NOT NULL,
  `Employee_ssn` INT(11) NOT NULL,
  PRIMARY KEY (`Property_propertyID`, `Employee_ssn`),
  INDEX `fk_Property_has_Employee_Employee1_idx` (`Employee_ssn` ASC),
  INDEX `fk_Property_has_Employee_Property1_idx` (`Property_propertyID` ASC),
  CONSTRAINT `fk_Property_has_Employee_Employee1`
  FOREIGN KEY (`Employee_ssn`)
  REFERENCES `adase1db`.`Employee` (`ssn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Property_has_Employee_Property1`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Expense`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Expense` (
  `expenseId` INT(11) NOT NULL,
  `cost` DECIMAL(13,2) NULL DEFAULT NULL,
  `expense` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`expenseId`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Repair`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Repair` (
  `repairId` INT(11) NOT NULL,
  `cost` DECIMAL(13,2) NULL DEFAULT NULL,
  `effectedArea` VARCHAR(45) NULL DEFAULT NULL,
  `vendor` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`repairId`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Property_Needs_Repair`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Property_Needs_Repair` (
  `Repair_repairId` INT(11) NULL DEFAULT NULL,
  `Property_propertyID` INT(11) NOT NULL,
  `Property_RepairID` INT(11) NOT NULL,
  PRIMARY KEY (`Property_RepairID`),
  INDEX `fk_Repair_has_Property_Property1` (`Property_propertyID` ASC),
  INDEX `fk_Repair_has_Repair_Repair1` (`Repair_repairId` ASC),
  CONSTRAINT `fk_Repair_has_Property_Property1`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Repair_has_Repair_Repair1`
  FOREIGN KEY (`Repair_repairId`)
  REFERENCES `adase1db`.`Repair` (`repairId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Property_has_Amenity`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Property_has_Amenity` (
  `Amenity_amenityid` INT(11) NOT NULL,
  `Property_propertyID` INT(11) NOT NULL,
  `Porporty_amenityid` INT(11) NOT NULL,
  PRIMARY KEY (`Porporty_amenityid`),
  INDEX `fk_Amenity_has_Property_Property1_idx` (`Property_propertyID` ASC, `Porporty_amenityid` ASC),
  INDEX `Property_Amenity_Has_Amenity_idx` (`Amenity_amenityid` ASC),
  CONSTRAINT `Property_Amenity_Has_Amenity`
  FOREIGN KEY (`Amenity_amenityid`)
  REFERENCES `adase1db`.`Amenity` (`amenityid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Property_Amenity_Has_Property`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Property_has_Expense`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Property_has_Expense` (
  `Expense_expenseId` INT(11) NULL DEFAULT NULL,
  `Property_propertyID` INT(11) NULL DEFAULT NULL,
  `Property_ExpenseID` INT(11) NOT NULL,
  PRIMARY KEY (`Property_ExpenseID`),
  INDEX `fk_Expense_has_Property_Expense1_idx` (`Expense_expenseId` ASC),
  INDEX `Property_Expense_Has_Property_idx` (`Property_propertyID` ASC),
  CONSTRAINT `Property_Expense_Has_Expense`
  FOREIGN KEY (`Expense_expenseId`)
  REFERENCES `adase1db`.`Expense` (`expenseId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Property_Expense_Has_Property`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Seller`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Seller` (
  `ssn` INT(11) NOT NULL,
  `zipCode` INT(11) NULL DEFAULT NULL,
  `streetAddress` VARCHAR(45) NULL DEFAULT NULL,
  `city` VARCHAR(45) NULL DEFAULT NULL,
  `State` VARCHAR(45) NULL DEFAULT NULL,
  `phoneNumber` VARCHAR(45) NULL DEFAULT NULL,
  `f_name` VARCHAR(45) NULL DEFAULT NULL,
  `m_name` VARCHAR(45) NULL DEFAULT NULL,
  `l_name` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`ssn`))
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


-- -----------------------------------------------------
-- Table `adase1db`.`Seller_Sells_Property`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adase1db`.`Seller_Sells_Property` (
  `Seller_ssn` INT(11) NOT NULL,
  `Property_propertyID` INT(11) NOT NULL,
  `price` DECIMAL(13,2) NULL DEFAULT NULL,
  PRIMARY KEY (`Seller_ssn`, `Property_propertyID`),
  INDEX `fk_Seller_has_Property_Seller1_idx` (`Seller_ssn` ASC),
  INDEX `fk_Seller_has_Property_Property1_idx` (`Property_propertyID` ASC),
  CONSTRAINT `fk_Seller_has_Property_Property1`
  FOREIGN KEY (`Property_propertyID`)
  REFERENCES `adase1db`.`Property` (`propertyID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Seller_has_Property_Seller1`
  FOREIGN KEY (`Seller_ssn`)
  REFERENCES `adase1db`.`Seller` (`ssn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB
  DEFAULT CHARACTER SET = latin1;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
