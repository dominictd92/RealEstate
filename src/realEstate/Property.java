...
public class Property extends javax.swing.JFrame {
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        try{
                ...
                String sql = "UPDATE Property SET deckOrPatio=?,washer=?,dryer=?,offStreetParking=?,numberOfBaths=?,buildingType=?,phoneNumber=?,basement=?,parkingType=?,numOfRooms=?,zipCode=?,streetAddress=?,city=?,state=?,numOfTennants=?,Tennant_ssn=? where propertyID="+value;
    ...
    private void btnInsertActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInsertActionPerformed
        ...
        try{
                Class.forName(DATABASE_CONSTANTS.DRIVER);
                Connection conn = DriverManager.getConnection(DATABASE_CONSTANTS.URL, DATABASE_CONSTANTS.USERNAME, DATABASE_CONSTANTS.PASSWORD);
                String sql = "Insert into Property (deckOrPatio,washer,dryer,offStreetParking,numberOfBaths,buildingType,phoneNumber,basement,parkingType,numOfRooms,zipCode,streetAddress,city,state,numOfTennants,Tennant_ssn) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                PreparedStatement pst = conn.prepareStatement(sql);
    ...
    private void btnDeleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleActionPerformed
        ...
        try{
                ...
                int row = tblProp.getSelectedRow();
                String value = (tblProp.getModel().getValueAt(row, 16).toString());
                String sql = "DELETE FROM Property where propertyID="+value;
                PreparedStatement pst = conn.prepareStatement(sql);
                pst.execute();
    ...
    private void Fill_BldgStyle() {
        try {
            Connection conn = DriverManager.getConnection(DATABASE_CONSTANTS.URL, DATABASE_CONSTANTS.USERNAME, DATABASE_CONSTANTS.PASSWORD);
            String sql="select * from Bldg_Style";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst=conn.prepareStatement(sql);
            rs=pst.executeQuery();
            while(rs.next())
    ...
    private void Fill_ParkingType() {
        try {
            Connection conn = DriverManager.getConnection(DATABASE_CONSTANTS.URL, DATABASE_CONSTANTS.USERNAME, DATABASE_CONSTANTS.PASSWORD);
            String sql="select * from Parking_Style";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst=conn.prepareStatement(sql);
            rs=pst.executeQuery();
            while(rs.next())
